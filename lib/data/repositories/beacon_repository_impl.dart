...
class BeaconRepositoryImpl implements BeconRepository {
  ...
  Future<void> _getBeaconFromCSV() async {
    _knownBeacons = {};
    try {
      final input = await rootBundle.loadString('assets/beacons.json');
      var json = jsonDecode(input) as List<dynamic>;
      for (var device in json) {
        _knownBeacons[device["Device_id"]] = device;
      }
    } catch (e) {
  ...
  void _deviceCallback(List<ScanResult> results) {
    ...
    _deviceStreamController.add(results
        .where((e) =>
            _knownBeacons.containsKey(e.device.remoteId.str) &&
  ...
  num _calculateRealDistance(String id, double mapScale, int n,
    ...
    if (_lastFilteredValue[id]! <= _knownBeacons[id]?['RSSI_at_1m']) {
      distanceMeters = _calculateMetersFromRssi(
          _lastFilteredValue[id]!, _knownBeacons[id]?['RSSI_at_1m'] as num,
          n: n);
    } else {
      distanceMeters = _calculateMetersFromRssi(
          _lastFilteredValue[id]!, _knownBeacons[id]?['RSSI_at_1m'] as num,
  ...
  List<num> _calculateCircleIntersection(
      num x1, num y1, num r1, num x2, num y2, num r2) {
    ...
    double test = ((xIntersection1 - x1) * (xIntersection1 - x1) +
  ...
  (double, double, int, double, bool) deviceLocation(
    ...
    for (int i = 0; i < foundBeacons.length && i < 6; i++) {
      if (_kalmanFilter.containsKey(foundBeacons[i].id)) {
        ...
        print(
            "Got: ${foundBeacons[i].rssi} filtered: ${_lastFilteredValue[foundBeacons[i].id]}");
      } else {
        _kalmanFilter[foundBeacons[i].id] =
    ...
    if (foundBeacons.length >= 3) {
      for (int i = 0; i < 3; i++) {
        ...
        intersectionPoints.add(_calculateCircleIntersection(
        ...
        while (_flagIntersectUp && defaultN + addToN > 3) {
          ...
          num newDistance1 = _calculateRealDistance(
              foundBeacons[i].id, mapScale, defaultN + addToN);
          num newDistance2 = _calculateRealDistance(
              foundBeacons[j].id, mapScale, defaultN + addToN);
          ...
          intersectionPoints[i] = _calculateCircleIntersection(
              x1, y1, newDistance1, x2, y2, newDistance2);
          addToN--;
        }
        ...
        if (_flagIntersectUp) {
          ...
          _flagIntersectUp = false;
          double d = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2));
          List<num> closesPointCircle1 = _calculateCircleIntersection(
              x1, y1, realDistance[i], x2, y2, d - realDistance[i]);
          List<num> closesPointCircle2 = _calculateCircleIntersection(
              x1, y1, d - realDistance[j], x2, y2, realDistance[j]);
          List<num> middlePoint = [
        ...
        if (_flagIntersectDown) {
          ...
          if (realDistance[i] > realDistance[j]) {
            List<num> closesPointCircle1 = _calculateCircleIntersection(
                x1, y1, d + realDistance[j], x2, y2, realDistance[j]);
            List<num> closesPointCircle2 = _calculateCircleIntersection(
                x1, y1, realDistance[i], x2, y2, realDistance[i] - d);
            List<num> middlePoint = [
              (closesPointCircle1[0] + closesPointCircle2[0]) / 2,
              (closesPointCircle1[1] + closesPointCircle2[1]) / 2
            ];
            ...
            List<num> closesPointCircle1 = _calculateCircleIntersection(
                x1, y1, realDistance[i], x2, y2, d + realDistance[i]);
            List<num> closesPointCircle2 = _calculateCircleIntersection(
                x1, y1, realDistance[j] - d, x2, y2, realDistance[j]);
            List<num> middlePoint = [
              (closesPointCircle1[0] + closesPointCircle2[0]) / 2,
              (closesPointCircle1[1] + closesPointCircle2[1]) / 2
            ];
      ...
      intersectionPoints.add(_calculateCircleIntersection(
          x1, y1, realDistance[0], x2, y2, realDistance[1]));
      ...
      while (_flagIntersectUp && defaultN + addToN > 0) {
        ...
        _flagIntersectUp = false;
        num newDistance1 = _calculateRealDistance(
            foundBeacons[0].id, mapScale, defaultN + addToN);
        num newDistance2 = _calculateRealDistance(
            foundBeacons[1].id, mapScale, defaultN + addToN);
      ...
      if (_flagIntersectUp) {
        ...
        double d = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2));
        List<num> closesPointCircle1 = _calculateCircleIntersection(
            x1, y1, realDistance[0], x2, y2, d - realDistance[0]);
        List<num> closesPointCircle2 = _calculateCircleIntersection(
            x1, y1, d - realDistance[1], x2, y2, realDistance[1]);
        List<num> middlePoint = [
          (closesPointCircle1[0] + closesPointCircle2[0]) / 2,
          (closesPointCircle1[1] + closesPointCircle2[1]) / 2
        ];
      ...
      if (_flagIntersectDown) {
        ...
        if (realDistance[0] > realDistance[1]) {
          List<num> closesPointCircle1 = _calculateCircleIntersection(
              x1, y1, d + realDistance[1], x2, y2, realDistance[1]);
          List<num> closesPointCircle2 = _calculateCircleIntersection(
              x1, y1, realDistance[0], x2, y2, realDistance[0] - d);
          List<num> middlePoint = [
            (closesPointCircle1[0] + closesPointCircle2[0]) / 2,
            (closesPointCircle1[1] + closesPointCircle2[1]) / 2
          ];
          ...
          List<num> closesPointCircle1 = _calculateCircleIntersection(
              x1, y1, realDistance[0], x2, y2, d + realDistance[0]);
          List<num> closesPointCircle2 = _calculateCircleIntersection(
              x1, y1, realDistance[1] - d, x2, y2, realDistance[1]);
          List<num> middlePoint = [
            (closesPointCircle1[0] + closesPointCircle2[0]) / 2,
            (closesPointCircle1[1] + closesPointCircle2[1]) / 2
          ];
